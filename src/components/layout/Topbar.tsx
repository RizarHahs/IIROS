          ...
          <DropdownMenuTrigger asChild>
            <div className="flex items-center gap-3 ml-2 pl-3 border-l border-border cursor-pointer hover:opacity-80 transition-opacity">
              <div className="hidden md:block text-right">
                <p className="text-xs font-medium text-foreground">{user?.name || "BKC Admin"}</p>
                <p className="text-[10px] text-muted-foreground">{user?.department || "Energy Manager"}</p>
              </div>
              <div className={`h-8 w-8 rounded-full flex items-center justify-center ${user?.role === 'admin' ? 'bg-gradient-to-br from-primary to-primary/60' : 'bg-gradient-to-br from-emerald-500 to-emerald-500/60'}`}>
                <span className="text-xs font-bold text-white">{user?.avatar || "BK"}</span>
              </div>
            </div>
          </DropdownMenuTrigger>
